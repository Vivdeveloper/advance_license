...
function _toggle_advance_license_column(frm) {
	...
	const hidden = !frm.doc.supplier ? 1 : 0;
	frappe.db.get_value("Supplier", frm.doc.supplier, "custom_advance_license_applicable", function(r) {
		if (frm.doc.supplier && r && r.custom_advance_license_applicable === "Yes") {
...
function _refresh_row_licenses(frm) {
	...
	frm.doc.items.forEach(function(row) {
		...
		const cdn = row.name || "Purchase Invoice Item-" + row.idx;
...
function _validate_license_qty(frm, throw_error) {
	if (!frm.doc.items || frm.doc.items.length === 0 || !frm.doc.supplier) {
		return true;
	}
